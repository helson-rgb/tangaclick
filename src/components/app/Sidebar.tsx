    ...
    <aside className="flex w-80 shrink-0 flex-col bg-brand-navy px-6 py-8">
      ...
      <div className="space-y-4 border-t border-white/10 pt-6">
        <div className="flex items-center gap-3 px-2">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-brand-orange font-heading text-sm text-white">
            {officina.operatore
              .split(" ")
              .map((part) => part[0])
              .join("")}
        ...
        <Link
          href="/app/login"
          className="flex items-center gap-3.5 rounded-xl px-4 py-3 text-[15px] font-medium text-white/50 transition-colors hover:bg-white/5 hover:text-white/90"
        >
